...
export function AccountLoginForm ({ callbackUrl }: AccountLoginFormProps)
{
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) =>
  {
    ...
    {
      const signInAction = auth.signIn ?? signIn;
      ...
      const getSessionAction = auth.getSession ?? getSession;
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-end">
          <Link
            ...
            className="text-xs text-purple-200 underline decoration-dotted underline-offset-4 hover:text-purple-100"
          >
            Forgot password?
          </Link>
        </div>
